class ModelGenerator
    ...
    def self.generate_models(yml : YAML::Any, folder : String)
      ...
      yml["components"]["schemas"].as_h.each do |key, value|
        ...
        when Hash
          ...
          if obj == "object"
            generate_model(key.to_s, value.as_h["properties"].as_h, folder)
          end
        else
    ...
    def self.generate_model(name : String, props : Hash(YAML::Any, YAML::Any), folder : String)
      ...
      file_name = "#{folder}/src/onchain-shard/models/#{name}.cr"
      Dir.mkdir_p("#{folder}/src/onchain-shard/models/")
      File.write(file_name, model_class)
    end
